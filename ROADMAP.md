...
- **Retention coupon at cancel (GR-3).** Offer a discount to customers about to cancel, via the Stripe customer portal (Cancellations → Retention coupons). The coupon is a real Stripe object — **create it in LIVE mode first** (e.g. a % off for N months), then attach it in the portal config. Not a launch blocker. **Trust-brand guardrail:** keep it a transparent offer — cancel stays one click away, easy to decline. A fair discount, never a dark pattern (consistent with the "success = the child disengaging / no engagement traps" ethic). Decide the offer (% + duration) sensible for a $8.99–12.99 product before creating.
