 ...
 * SINGLE ATTEMPT — no retry. Two rapid POST /sign-in requests back-to-back can
 * trip auth rate-limiting, so we fire exactly one. On failure we surface the
 * reason (rate limit / bad creds / slow redirect) instead of an opaque timeout.
 */
export async function login(page: Page, email: string, password: string, timeout = 15000) {
  ...
  try {
    ...
    throw new AssertionError(
      `login did not reach /dashboard within ${timeout}ms (check credentials in .env.local)${diag}`,
    );
  }
}
