...
type PageSpeedRow = {
  path: string; performanceScore: number | null;
  lcpMs: number | null; tbtMs: number | null; ttfbMs: number | null; createdAt: string;
};
type Product = {
  ...
  pageSpeed: { latest: PageSpeedRow[]; runs: PageSpeedRow[] };
};

// Mirrors CONFIG.pageSpeed in src/app/api/cron/health-check/route.ts. Used only
// to colour the numbers here — the alert itself is decided there.
...
function ProductPanel({ p }: { p: Product }) {
  ...
  return (
    <>
      ...
      <PageSpeedPanel latest={p.pageSpeed?.latest ?? []} runs={p.pageSpeed?.runs ?? []} />
    </>
  );
}

// ---------- page speed (outside-in, written by the daily health check) --------
// Two views of the same table: where we stand right now, and the week behind it.
// Numbers past a threshold go red — the same thresholds the cron alerts on.
function PageSpeedPanel({ latest, runs }: { latest: PageSpeedRow[]; runs: PageSpeedRow[] }) {
  const bad = "text-[#9a3b2e] font-semibold";

  return (
    <div className="mt-7">
      <h3 className="text-[15px] font-semibold text-ink">Page speed</h3>
      <p className="mt-0.5 mb-3 text-[13px] text-ink-muted">
        PageSpeed Insights, mobile, measured every morning at 7am PT. Alerts fire above{" "}
        {(SPEED_LIMITS.maxLcpMs / 1000).toFixed(1)}s LCP or below a score of {SPEED_LIMITS.minScore}.
      </p>

      {runs.length === 0 ? (
        <p className="rounded-2xl border border-[#e8d9b5] bg-[#fdf6e6] px-4 py-3 text-[13px] text-ink">
          <strong>No measurements yet.</strong> These fill from{" "}
          <code className="font-mono text-[12px]">page_speed</code>, written by the health-check cron
          on its next 7am run.
        </p>
      ) : (
        <>
              ...
              {latest.map((r) => (
                <tr key={r.path}>
                  <td className={TD}>
                    ...
                    <span className="block text-[12px] text-ink-muted">
                      LCP <span className={r.lcpMs !== null && r.lcpMs > SPEED_LIMITS.maxLcpMs ? bad : ""}>{ms(r.lcpMs)}</span>
                      {"  ·  TBT "}{ms(r.tbtMs)}
                      {"  ·  TTFB "}{ms(r.ttfbMs)}
                      {"  ·  "}{day(r.createdAt)}
                    </span>
                  </td>
                  <td className={`${TDNUM} w-[140px] text-[18px] ${r.performanceScore !== null && r.performanceScore < SPEED_LIMITS.minScore ? bad : "font-semibold"}`}>
                    {r.performanceScore ?? "—"}
                  </td>
                </tr>
              ))}
          ...
          <p className="mt-5 mb-2 text-[12px] font-bold uppercase tracking-wide text-ink-muted">
            Last 7 days
          </p>
          <Table>
            <thead>
              <tr>
                <th className={TD}>Day</th>
                <th className={TD}>Page</th>
                <th className={TDNUM}>Score</th>
                <th className={TDNUM}>LCP</th>
                <th className={TDNUM}>TBT</th>
                <th className={TDNUM}>TTFB</th>
              </tr>
            </thead>
            <tbody>
              {runs.map((r, i) => (
                <tr key={`${r.path}-${r.createdAt}-${i}`}>
                  <td className={TD}>{day(r.createdAt)}</td>
                  <td className={`${TD} font-mono`}>{r.path}</td>
                  <td className={`${TDNUM} ${r.performanceScore !== null && r.performanceScore < SPEED_LIMITS.minScore ? bad : ""}`}>
                    {r.performanceScore ?? "—"}
                  </td>
                  <td className={`${TDNUM} ${r.lcpMs !== null && r.lcpMs > SPEED_LIMITS.maxLcpMs ? bad : ""}`}>{ms(r.lcpMs)}</td>
                  <td className={TDNUM}>{ms(r.tbtMs)}</td>
                  <td className={TDNUM}>{ms(r.ttfbMs)}</td>
                </tr>
              ))}
            </tbody>
          </Table>
      ...
      <div className="mx-auto w-full max-w-6xl">
        ...
        {m && (
          <>
            ...
            <Section title="Product health" sub="The last 24 hours of story generation, and how the public pages are performing.">
