...
export type ProductHealth = {
  ...
  /** Outside-in page speed, last 7 days. Written by the daily health check. */
  pageSpeed: PageSpeedHistory;
};

export async function getProductHealth(): Promise<ProductHealth> {
  const db = getDb();
  const [usage, api, demo, pageSpeed] = await Promise.all([
    db.execute(sql`
      select
...
// --- daily health check ------------------------------------------------------
// Read models for /api/cron/health-check. The SQL lives here with the rest of
// the metrics SQL rather than in the route, so there is one place that knows how
// api_events and page_speed are shaped.
...
const speedRow = (r: Row): PageSpeedRow => ({
  path: String(r.path ?? ""),
  performanceScore: r.performance_score === null || r.performance_score === undefined ? null : num(r.performance_score),
  lcpMs: r.lcp_ms === null || r.lcp_ms === undefined ? null : num(r.lcp_ms),
  tbtMs: r.tbt_ms === null || r.tbt_ms === undefined ? null : num(r.tbt_ms),
  ttfbMs: r.ttfb_ms === null || r.ttfb_ms === undefined ? null : num(r.ttfb_ms),
  // Postgres `timestamp` has no zone marker and is stored UTC — say so, so the
  // browser doesn't read it as local time.
  createdAt: (() => {
    const v = String(r.created_at ?? "");
    return /[Zz]|[+-]\d{2}:?\d{2}$/.test(v) ? v : `${v.replace(" ", "T")}Z`;
  })(),
});

export type PageSpeedHistory = {
  /** Most recent run per page — "where we stand today". */
  latest: PageSpeedRow[];
  /** Every run in the last 7 days, newest first — the trend behind it. */
  runs: PageSpeedRow[];
};

export async function getPageSpeed7d(): Promise<PageSpeedHistory> {
  const db = getDb();
  const [latest, runs] = await Promise.all([
    db.execute(sql`
      select distinct on (path) path, performance_score, lcp_ms, tbt_ms, ttfb_ms, created_at
      from page_speed
      where created_at >= now() - interval '7 days'
      order by path, created_at desc
    `),
    db.execute(sql`
      select path, performance_score, lcp_ms, tbt_ms, ttfb_ms, created_at
      from page_speed
      where created_at >= now() - interval '7 days'
      order by created_at desc, path
    `),
  ]);
  return { latest: rows(latest).map(speedRow), runs: rows(runs).map(speedRow) };
}

export type GenerationErrors = { total: number; errors: number };

/**
 * Story-generation requests and how many of them failed, over a rolling window.
 * Status codes come from the caller's CONFIG so the health check's thresholds
 * stay in one place — they are coerced to integers before reaching the SQL.
 */
export async function getGenerationErrors(
  windowHours: number,
  errorStatuses: number[],
  serverErrorFrom: number
): Promise<GenerationErrors> {
  const db = getDb();
  const hours = sql.raw(String(Math.trunc(windowHours)));
  const list = sql.raw(errorStatuses.map((n) => String(Math.trunc(n))).join(", ") || "-1");
  const from = sql.raw(String(Math.trunc(serverErrorFrom)));

  const r = rows(
    await db.execute(sql`
      select count(*)::int as total,
             count(*) filter (where status in (${list}) or status >= ${from})::int as errors
      from api_events
      where route = 'generate-story'
        and created_at >= now() - interval '${hours} hours'
    `)
  )[0] ?? {};

  return { total: num(r.total), errors: num(r.errors) };
}

export type NightlyCronHealth = {
  /** Run markers the nightly cron left in api_events. */
  runs: number;
  /** Nightly stories actually written in the window. */
  delivered: number;
  activeSubscriptions: number;
};

export async function getNightlyCronHealth(windowHours: number): Promise<NightlyCronHealth> {
  const db = getDb();
  const hours = sql.raw(String(Math.trunc(windowHours)));
  ...
  const r = rows(
    await db.execute(sql`
      select
        (select count(*) from api_events
          where route = 'cron-nightly-stories'
            and created_at >= now() - interval '${hours} hours')::int as runs,
        (select count(*) from stories
          where is_nightly = true
            and created_at >= now() - interval '${hours} hours')::int as delivered,
        (select count(*) from subscriptions
          where status in ('active','trialing'))::int as active_subs
    `)
  )[0] ?? {};
